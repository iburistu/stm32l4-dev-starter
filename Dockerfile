...
RUN sudo apt-get install gcc g++ autoconf automake libusb-1.0-0-dev gcc-arm-none-eabi libnewlib-arm-none-eabi make wget unzip -y && sudo wget -N http://web.eece.maine.edu/~hummels/classes/ece486/src/stmdevel.zip && sudo unzip -d stmdevel stmdevel.zip && cd stmdevel/build && sudo make && sudo make install 

# Second stage: copy all built files into a clean ubuntu image and minimize dependencies
FROM ubuntu:latest
RUN apt-get update && apt-get install gcc-arm-none-eabi libnewlib-arm-none-eabi make -y && rm -rf /var/lib/apt/lists/*
